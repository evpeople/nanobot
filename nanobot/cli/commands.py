...
def gateway(
    ...
    async def _init_gateway_services():
        from nanobot.dreamlife.service import DreamLifeService as DreamSvc
        ...
        if config.memory.enabled:
            openviking_client = OpenVikingClient(storage_path=config.memory.storage_path)
            await openviking_client.initialize()
            memory_service = MemSvc(config=config.memory, client=openviking_client)
            console.print(
                f"[green]✓[/green] Memory service initialized (storage: {config.memory.storage_path})"
            )

        # Initialize PersonalityService
        if config.personality:
            personality_service = PersSvc(config=config.personality)
            console.print(
                f"[green]✓[/green] Personality service initialized ({config.personality.name})"
            )

        # Initialize DreamLifeService if enabled
        if config.dreamlife and config.dreamlife.enabled:
            openviking_client = (
                memory_service.client
                if memory_service
                else OpenVikingClient(storage_path=config.dreamlife.storage_path)
            )
            if not memory_service:
                await openviking_client.initialize()
            dreamlife_service = DreamSvc(
                config=config.dreamlife, openviking_client=openviking_client
            )
            await dreamlife_service.initialize()
            console.print("[green]✓[/green] DreamLife service initialized")

        # Initialize ProactiveService if enabled
        if config.memory.proactive_enabled and memory_service:
            # Create a message sender for proactive service
            class _ProactiveSender:
            ...
            proactive_sender = _ProactiveSender(bus, None)
    ...
    agent = AgentLoop(
        ...
        dreamlife_service=dreamlife_service,
    )
...
def agent(
    ...
    # Initialize AI Girlfriend services (async for agent command)
    memory_service = None
    personality_service = None
    dreamlife_service = None
